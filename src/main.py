...
import time
import pandas as pd
import torch  # 確保引入 torch
import schedule
from ricxappframe.xapp_frame import Xapp, rmr
from ricxappframe.xapp_sdl import SDLWrapper
from mdclogpy import Logger
from ad_model import modelling, CAUSE,GRUModelInference
...
def entry(self):
    ...
    load_model()
     ...
     # 每秒進行模型推論
    logger.info("設置每分執行模型推論...")
    schedule.every(60).seconds.do(periodic_inference)
...
def periodic_inference():
    ...
    try:
        ...
        # 初始化推論類別
        model_path = "gru_model.onnx"
        gru_inference = GRUModelInference(model_path, scaler)

        # 選取測試資料進行推論
        X_test = X_tensor.numpy()  # 轉為 NumPy 格式
        predictions_inverse = gru_inference.predict(X_test)

        # 輸出結果
        logger.info("每秒模型預測結果(遊樂設施等待時間): {}".format(predictions_inverse))
    except Exception as e:
        logger.error("模型推論時發生錯誤: {}".format(e))
