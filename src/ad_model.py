...
class GRUModelInference:
    def __init__(self, model_path, scaler):
        """
        初始化推論類別，載入 ONNX 模型與標準化工具。
        :param model_path: ONNX 模型檔案的路徑
        :param scaler: 用於標準化與反標準化的工具
        """
    ...
    def predict(self, X_test):
        """
        進行模型推論。
        :param X_test: 測試數據 (NumPy 格式)
        :return: 模型反標準化後的預測結果
        """
        predictions = self.session.run([self.output_name], {self.input_name: X_test})
        predictions = np.array(predictions[0])
        # logger("模型預測結果:", predictions)
